...
// ── DashboardSocial ───────────────────────────────────────────────────────────
function DashboardSocial({ data, er, territorioSel, busca, areaInicial }: {
  data: SocialRow[]; er: boolean; territorioSel: string | null; busca: string; areaInicial?: string
}) {
  ...
  const hasAreaData = useMemo(() => {
    if (!areaInicial) return true
    return data.some(d => (!territorioSel || d.regiao === territorioSel) && d.area === areaInicial)
  }, [data, territorioSel, areaInicial])

  const filtered = useMemo(() => {
    let rows = data
    if (territorioSel) rows = rows.filter(d => d.regiao === territorioSel)
    if (areaInicial) rows = rows.filter(d => d.area === areaInicial)
    ...
    return rows
  }, [data, territorioSel, areaInicial, statusFiltro, tipoFiltro, busca])
  ...
  if (!hasAreaData) {
    return (
      <div style={{ padding: '40px 0', textAlign: 'center' }}>
        <p style={{ fontFamily: dmMono.style.fontFamily, fontSize: '0.75rem', color: MUTED, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
          Dados em atualização · {areaInicial}
        </p>
      </div>
    )
  }
...
// ── DashboardUrbanismo ────────────────────────────────────────────────────────
function DashboardUrbanismo({ data, er, territorioSel, busca, areaInicial }: {
  data: UrbanismoRow[]; er: boolean; territorioSel: string | null; busca: string; areaInicial?: string
}) {
  const [statusFiltro, setStatusFiltro] = useState('Todos')
  ...
  const hasAreaData = useMemo(() => {
    if (!areaInicial) return true
    return data.some(d => (!territorioSel || d.territorio === territorioSel) && d.area === areaInicial)
  }, [data, territorioSel, areaInicial])
  ...
  const filtered = useMemo(() => {
    ...
    return rows
  }, [data, territorioSel, areaInicial, statusFiltro, tipologiaFiltro, busca])
...
export function DashboardPCI({
  ...
  const [territorioSel, setTerritorioSel] = useState<string | null>(territorioInicial ?? null)
  ...
  if (loading) {
    return (
      <div style={{ background: modoEmbutido ? 'transparent' : BG, minHeight: modoEmbutido ? undefined : '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Spin />
      </div>
    )
  }
  ...
  const innerPadding = modoEmbutido ? '0' : '0 32px'
  const innerMaxWidth = modoEmbutido ? '100%' : 1400
  const innerMargin = modoEmbutido ? 0 : '0 auto'

  return (
    <div style={{
      background: modoEmbutido ? 'transparent' : BG,
      ...(modoEmbutido ? {} : { minHeight: '100vh', display: 'flex', flexDirection: 'column' }),
    }}>
      <style>{`
      ...
      <div style={{ maxWidth: innerMaxWidth, margin: innerMargin, width: '100%', padding: innerPadding, boxSizing: 'border-box', ...(modoEmbutido ? {} : { flex: 1, display: 'flex', flexDirection: 'column' }) }}>
        ...
        <div style={{ padding: modoEmbutido ? '0 0 12px' : '12px 0 0' }}>
          <input
            value={busca}
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${BORDER}`, marginTop: modoEmbutido ? 8 : 20 }}>
        ...
        <div style={{ padding: '28px 0 48px' }}>
          {aba === 'social'
            ? <DashboardSocial data={socialData} er={er} territorioSel={territorioSel} busca={busca} areaInicial={areaInicial} />
            : <DashboardUrbanismo data={urbanismoData} er={er} territorioSel={territorioSel} busca={busca} areaInicial={areaInicial} />
          }
        </div>
      </div>
