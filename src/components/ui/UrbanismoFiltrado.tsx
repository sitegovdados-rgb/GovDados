    ...
    <div>
      ...
      <div className="pci-card p-5 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          {[
            { label: 'Buscar', value: busca, onChange: setBusca, isInput: true },
          ].map((f, i) => (
            <div key={i}>
              ...
              <input placeholder="Nome da intervenção..." value={f.value} onChange={e => f.onChange(e.target.value)}
          ...
          {filtrados.map((u: any) => (
            <div key={u.id} className="pci-card flex flex-col" style={{ borderLeft: `4px solid ${u.status === 'Executado' ? 'var(--pci-green)' : 'var(--pci-cyan)'}` }}>
              ...
              <div style={{ padding: '12px 20px', flex: 1 }}>
                <p style={{ fontFamily: 'Plus Jakarta Sans', fontSize: '0.78rem', color: 'var(--pci-dim)', lineHeight: 1.6 }}>{u.descricao}</p>
              </div>
