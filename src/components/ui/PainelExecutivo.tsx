...
function TabPopulacao({ indicadores }: { indicadores: any[] }) {
  ...
  const areasData = useMemo(() =>
    toChart(groupBy(indicadores, 'area_tematica'), 'area', 'qtd')
      .map(d => ({ ...d, area: shortLabel(String(d.area), 20) })),
    [indicadores])
...
function TabProgramas({ programas }: { programas: any[] }) {
  const eixoData   = useMemo(() => toChart(groupBy(programas, 'eixo'), 'eixo', 'qtd').map(d => ({ ...d, eixo: shortLabel(String(d.eixo), 22) })), [programas])
  const statusData = useMemo(() => toChart(groupBy(programas, 'status'), 'name', 'value'), [programas])
  const terrData   = useMemo(() => toChart(groupBy(programas, 'territorio'), 'terr', 'qtd').map(d => ({ ...d, terr: shortLabel(String(d.terr), 20) })), [programas])
...
function TabUrbanismo({ urbanismo }: { urbanismo: any[] }) {
  const tipoData   = useMemo(() => toChart(groupBy(urbanismo, 'tipo'), 'tipo', 'qtd').map(d => ({ ...d, tipo: shortLabel(String(d.tipo), 22) })), [urbanismo])
  const statusData = useMemo(() => toChart(groupBy(urbanismo, 'status'), 'name', 'value'), [urbanismo])
  const terrData   = useMemo(() => toChart(groupBy(urbanismo, 'territorio'), 'terr', 'qtd').map(d => ({ ...d, terr: shortLabel(String(d.terr), 20) })), [urbanismo])
...
function TabEquipamentos({ equipamentos }: { equipamentos: any[] }) {
  ...
  const terrData = useMemo(() => toChart(groupBy(equipamentos, 'territorio'), 'terr', 'qtd').map(d => ({ ...d, terr: shortLabel(String(d.terr), 20) })), [equipamentos])
