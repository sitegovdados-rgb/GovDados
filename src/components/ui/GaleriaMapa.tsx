...
function toProxyUrl(url: string): string {
  ...
  if (match) return `/api/imagem?id=${match[1]}`
  return url
}
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', borderRadius: 12, overflow: 'hidden', background: '#f0f4f8', border: '1px solid var(--pci-border)' }}>
        <img
          src={toProxyUrl(mapas[ativo].url)}
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        {mapas.map((mapa, i) => (
          <button key={i} onClick={() => setAtivo(i)} style={{
            ...
            <img src={toProxyUrl(mapa.url)} alt={mapa.titulo} style={{ width: '100%', height: 64, objectFit: 'cover', display: 'block' }} />
            <p style={{
