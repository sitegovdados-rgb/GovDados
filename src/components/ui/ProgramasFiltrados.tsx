...
export default function ProgramasFiltrados({ programas }: Props) {
  ...
  const eixos = useMemo(() =>
    ['Todos', ...Array.from(new Set(programas.map((p) => p.eixo || 'Social'))).sort()],
    [programas]
  )

  const filtrados = useMemo(() => programas.filter(p => {
    const eixo = p.eixo || 'Social'
    if (eixoFiltro !== 'Todos' && eixo !== eixoFiltro) return false
    if (terrFiltro !== 'Todos') {
      const terr = [p.territorio, p.sub_territorio].join(' ')
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Realizados', valor: filtrados.filter(p => p.status === 'Realizado').length },
      ...
      <div className="pci-card p-5 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <div>
            ...
            <input type="text" placeholder="Nome do programa..." value={busca} onChange={e => setBusca(e.target.value)}
          ...
          <div>
            ...
            <select value={terrFiltro} onChange={e => setTerrFiltro(e.target.value)}
              style={{ width: '100%', padding: '8px 12px', fontSize: '0.85rem', border: '1px solid var(--pci-border)', borderRadius: 6, outline: 'none', fontFamily: 'Plus Jakarta Sans', background: 'var(--pci-bg)', color: 'var(--pci-text)' }}>
              <option>Todos</option>
              {TERRITORIOS_FIXOS.map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={() => { setBusca(''); setEixoFiltro('Todos'); setTerrFiltro('Todos'); setStatusFiltro('Todos') }}
      ...
      {verGrafico && dadosGrafico.length > 0 && (
        <div className="pci-card p-6 mb-8">
          ...
          <p style={{ fontFamily: 'JetBrains Mono', fontSize: '0.6rem', color: 'var(--pci-muted)', marginBottom: 20, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Top {dadosGrafico.length} · Filtro atual
          </p>
              ...
              <div key={p.id} className="pci-card flex flex-col" style={{ borderTop: `3px solid ${cor}`, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '12px 20px', flex: 1 }}>
                  <p style={{ fontFamily: 'Plus Jakarta Sans', fontSize: '0.78rem', color: 'var(--pci-dim)', lineHeight: 1.6 }}>
                    {p.descricao?.substring(0, 110)}{p.descricao?.length > 110 ? '…' : ''}
                  </p>
                </div>

                {/* Rodapé */}
                <div style={{ padding: '10px 20px 16px', borderTop: '1px solid var(--pci-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 8 }}>
                  <div>
                    {p.beneficiarios ? (
                      <>
                  ...
                  {SLUGS_PROGRAMAS[p.titulo] && (
                <Link href={SLUGS_PROGRAMAS[p.titulo]} style={{ fontFamily: 'JetBrains Mono', fontSize: '0.58rem', color: 'var(--pci-blue)', textTransform: 'uppercase', letterSpacing: '0.06em', textDecoration: 'underline' }}>
                  Ver detalhes →
                </Link>
              )}
              {p.territorio && (
                    <p style={{ fontFamily: 'JetBrains Mono', fontSize: '0.58rem', color: 'var(--pci-muted)', textAlign: 'right', lineHeight: 1.4 }}>
